/// Prevents memory leaks, very unsafe unless you know that all copies of this reference are dropped before/with this
/// this means that all returned references to this static should not last longer than this object
#[derive(Debug)]
pub struct StaticRefHolder<T> where T: 'static {
    reference: *mut T
}
...
impl<T> Drop for StaticRefHolder<T> {
    fn drop(&mut self) {
        unsafe { Box::from_raw(self.reference); }
    }
}
